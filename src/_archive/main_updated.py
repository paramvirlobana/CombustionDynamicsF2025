...
def process_design_point(design_params, PHI_range, M_range, BR_range=None, progress_queue=None):
    """
    ...
    for items in iterator:
        ...
        results['tau_mix'][local_iter] = results['L_premixer'][local_iter] / U0
...
def compute_design_space(args):
    ...
    # ------------- MODE JET FLAMES -------------
    print("----- JET STABILIZED FLAMES -----")
    print(f"Total design points: {design_points_jet}")
    num_processes = min(cpu_count(), len(U0_range))
    print(f"Using {num_processes} CPU cores.")
    print("------------------")
    
    # Prepare parameters for each U0 value
    points_per_u0_jet = len(PHI_range) * len(M_range)
    design_params_jet = [(U0, i * points_per_u0_jet, i) for i, U0 in enumerate(U0_range)]
    ...
    for i, U0 in enumerate(U0_range):
        progress_bars_jet[i] = tqdm(
            total=points_per_u0_jet,
    ...
    monitor_thread = threading.Thread(
        ...
        args=(progress_queue, progress_bars_jet, stop_event)
    )
    monitor_thread.start()
    
    process_func_jet = partial(process_design_point, PHI_range=PHI_range, M_range=M_range, BR_range=None, progress_queue=progress_queue)
    
    with Pool(processes=num_processes) as pool:
        results_jet = pool.map(process_func_jet, design_params_jet)
    
    stop_event.set()
    monitor_thread.join()
    
    for pbar in progress_bars_jet.values():
        pbar.close()
    
    print("\nJet flame processes completed!")
    print("------------------")
    
    combined_data_jet = {
        key: np.concatenate([r[key] for r in results_jet])
        for key in results_jet[0].keys()
    }
    
    df_jet = pd.DataFrame({
        'Stabilization': ['Jet Flame'] * design_points_jet,
        'U0': combined_data_jet['U0'],
        'PHI': combined_data_jet['PHI'],
        'M': combined_data_jet['M'],
        'BR': np.nan,
        'T_ad': combined_data_jet['T_ad'],
        'm_dot_air': combined_data_jet['m_dot_air'],
        'D': combined_data_jet['D'],
        'tau_mix': combined_data_jet['tau_mix'],
        'eta': combined_data_jet['eta'],
        'L_premixer': combined_data_jet['L_premixer'],
        'x_injector': combined_data_jet['x_injector'],
        'D_BB': np.nan,
        'S_L': combined_data_jet['S_L'],
        'delta_f': combined_data_jet['delta_f'],
        'lt_Lf': combined_data_jet['lt_Lf'],
        'ui_SL': combined_data_jet['ui_SL'],
        'ST_SL_Peters': combined_data_jet['ST_SL_Peters'],
        'ST_SL_Zimont': combined_data_jet['ST_SL_Zimont'],
        'ST_SL_Muppala': combined_data_jet['ST_SL_Muppala'],
        'dP_grid': combined_data_jet['dP_grid'],
        'dP_fric': combined_data_jet['dP_fric'],
        'dP_BB': np.nan,
        'dP_total': combined_data_jet['dP_total'],
        'dP_percent': combined_data_jet['dP_percent']
    })

    # ------------- MODE BLUFF-BODY FLAMES -------------
    print("----- BLUFF-BODY STABILIZED FLAMES -----")
    print(f"Total design points: {design_points_bluff}")
    print(f"Using {num_processes} CPU cores.")
    print("------------------")
    
    points_per_u0_bluff = len(PHI_range) * len(M_range) * len(BR_range)
    design_params_bluff = [(U0, i * points_per_u0_bluff, i) for i, U0 in enumerate(U0_range)]
    
    progress_bars_bluff = {}
    for i, U0 in enumerate(U0_range):
        progress_bars_bluff[i] = tqdm(
            total=points_per_u0_bluff,
            desc=f"U0={U0:>6.1f} m/s",
            position=i,
            leave=True,
            ncols=100,
    ...
    stop_event_bluff = threading.Event()
    monitor_thread_bluff = threading.Thread(
        target=progress_monitor,
        args=(progress_queue, progress_bars_bluff, stop_event_bluff)
    )
    monitor_thread_bluff.start()
    
    process_func_bluff = partial(process_design_point, PHI_range=PHI_range, M_range=M_range, BR_range=BR_range, progress_queue=progress_queue)
    
    with Pool(processes=num_processes) as pool:
        results_bluff = pool.map(process_func_bluff, design_params_bluff)
    
    stop_event_bluff.set()
    monitor_thread_bluff.join()
    
    for pbar in progress_bars_bluff.values():
        pbar.close()
    
    print("\nBluff-body flame processes completed!")
    print("------------------")
    
    combined_data_bluff = {
        key: np.concatenate([r[key] for r in results_bluff])
        for key in results_bluff[0].keys()
    }
    
    df_bluff = pd.DataFrame({
        'Stabilization': ['Bluff Body Flame'] * design_points_bluff,
        'U0': combined_data_bluff['U0'],
        'PHI': combined_data_bluff['PHI'],
        'M': combined_data_bluff['M'],
        'BR': combined_data_bluff['BR'],
        'T_ad': combined_data_bluff['T_ad'],
        'm_dot_air': combined_data_bluff['m_dot_air'],
        'D': combined_data_bluff['D'],
        'tau_mix': combined_data_bluff['tau_mix'],
        'eta': combined_data_bluff['eta'],
        'L_premixer': combined_data_bluff['L_premixer'],
        'x_injector': combined_data_bluff['x_injector'],
        'D_BB': combined_data_bluff['D_BB'],
        'S_L': combined_data_bluff['S_L'],
        'delta_f': combined_data_bluff['delta_f'],
        'lt_Lf': combined_data_bluff['lt_Lf'],
        'ui_SL': combined_data_bluff['ui_SL'],
        'ST_SL_Peters': combined_data_bluff['ST_SL_Peters'],
        'ST_SL_Zimont': combined_data_bluff['ST_SL_Zimont'],
        'ST_SL_Muppala': combined_data_bluff['ST_SL_Muppala'],
        'dP_grid': combined_data_bluff['dP_grid'],
        'dP_fric': combined_data_bluff['dP_fric'],
        'dP_BB': combined_data_bluff['dP_BB'],
        'dP_total': combined_data_bluff['dP_total'],
        'dP_percent': combined_data_bluff['dP_percent']
    })

    df_total = pd.concat([df_jet, df_bluff], ignore_index=True)
