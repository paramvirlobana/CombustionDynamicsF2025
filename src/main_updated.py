...
def main(
    ...
    for (U0, PHI, xM), _ in product(*design_variables):
        ...
        ## Check if maximum premixer length has small variance.
        #if eta > 0.28:
        #    iter += 1
        #    continue
            ## If variance is too high, move to the next iteration.

        # ===============================
        # PRESSURE LOSS CALCULATION
        # ===============================

        # Gas properties at inlet (T_30, P_30)
        gas_tmp = ct.Solution(mech)
        gas_tmp.TP = T_30, P_30
        rho = gas_tmp.density_mass               # density [kg/m3]
        mu  = gas_tmp.viscosity                  # dynamic viscosity [Pa·s]
        ...
        # Grid solidity (NOTE ASSUMPTION: choose sigma = 0.40)
        sigma = 0.40
        ...
        # Grid pressure drop
        DeltaP_grid = 0.5 * rho * U**2 * C_D * sigma

        # Frictional pressure drop along premixer length L
        DeltaP_fric = f_D * (L[iter] / D[iter]) * 0.5 * rho * U**2
